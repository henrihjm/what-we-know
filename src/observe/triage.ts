...
import { pLimit, warn, errMsg } from "../util.js";
...
async function ask(client: OpenAI, model: string, claims: CompactClaim[], snippet: string) {
  const claimList = claims.length ? claims.map((c) => `${c.id}: ${c.text}`).join("\n") : "(no claims yet)";
...
export async function triageOne(claims: CompactClaim[], snippet: string): Promise<{ label: TriageLabel; labelRaw: string; model: string; ms: number; usage: TokenUsage }> {
  ...
  fallbackClient ??= new OpenAI({ apiKey: env.OPENAI_API_KEY, maxRetries: 1 });
  const model = "gpt-4.1-mini";
  const r = await ask(fallbackClient, model, claims, snippet);
  const ms = Date.now() - started;
...
export async function triageSnippets(claims: CompactClaim[], snippets: Snippet[], budgetLeft: () => boolean): Promise<{ triaged: TriagedSnippet[]; usage: TokenUsage[]; skipped: number }> {
    ...
    snippets.map((s) =>
      limit(async (): Promise<TriagedSnippet | null> => {
        ...
        try {
          ...
          warn(`triage failed for ${s.id}: ${errMsg(e)}`);
          return { ...s, label: { kind: "new" }, labelRaw: "error->new", model: "none", ms: 0 };
        }
      }),
    ),
  );
  return { triaged: triaged.filter((t): t is TriagedSnippet => t !== null), usage, skipped };
}
